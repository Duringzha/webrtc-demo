    ...
    <div class="main">
        ...
        <div class="video-wrap">
            <div class="remoteVideoBox">
                <video class="remoteVideo" id="remoteVideo" playsinline autoplay muted/>
...
export default {
    ...
    sockets: {
            ...
            if(inviteInfo.user_id != _this.user_id){
                _this.$confirm('用户：'+ inviteInfo.user_id + '发起了视频聊天邀请，是否参加？', '提示', {
                    ...
                    navigator.mediaDevices.getUserMedia({
                        ...
                        localVideo.srcObject = _this.local_stream;
        ...
        newProductor(connector){
            ...
            _this.peerConnection.createOffer({ 
                ...
                _this.$socket.emit('call', {callee:_this.connector_id, sdp:offer});
            });
        },
        ...
        icecandidate(ice){
            const _this = this;
            if(_this.user_id != ice.owner){
                _this.connector_id = ice.owner;
                try {
                    console.info('addIceCandidate ', ice.candidate);
                    _this.re_peerConnection.addIceCandidate(ice.candidate).then(() => {
        ...
        offer(remoteOffer){
            ...
            _this.re_peerConnection.onicecandidate = event => {
                if (event.candidate && _this.connector_id){
                    console.log("re_peerConnection onicecandidate");
                    _this.$socket.emit('re_icecandidate', {
                        peer:_this.connector_id, 
                        owner:_this.user_id, 
                        candidate:event.candidate
                    })
            ...
            for (const track of _this.local_stream.getTracks()) {
            ...
            _this.re_peerConnection.createAnswer({ offerToReceiveAudio: true, offerToReceiveVideo: true}).then(answer => {
    ...
    methods: {
        ...
        _audioCtrl(){
            const _this = this;
            let videoTracks = _this.local_stream.getAudioTracks();
            if(_this.audio == "on"){
                _this.audio = "off";
                videoTracks[0].enabled = false;
            }else{
        ...
        _setRemoteVideo(ev){
            ...
            if(videoTracks.length){
                ...
                console.log("start stream play");
                remoteVideo.play();
            }
            let audioTracks = remote_stream.getAudioTracks();
